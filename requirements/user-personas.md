...
flowchart TB
    subgraph Brands["Brand Users"]
        B1[Experienced Brand]
        B2[New Brand]
        B3[Agency]
    end

    subgraph Creators["Creator Users"]
        C1[Professional Creator]
        C2[Emerging Creator]
        C3[Specialist Creator]
    end
    ...
    B1 <--> |"auto-accept ↔ reliable"| C1
    B2 <--> |"guided ↔ building reputation"| C2
    B3 <--> |"multi-brand ↔ niche premium"| C3

    B1 & B2 & B3 --> |"trust & disputes"| PA
    C1 & C2 & C3 --> |"trust & disputes"| PA

    %% Node Styles
    classDef brandNode fill:#ede9fe,stroke:#c4b5fd,stroke-width:2px,color:#4c1d95
    classDef creatorNode fill:#f3e8ff,stroke:#d8b4fe,stroke-width:2px,color:#581c87
    classDef adminNode fill:#1e293b,stroke:#1e293b,stroke-width:2px,color:#ffffff

    %% Apply styles
    class B1,B2,B3 brandNode
    class C1,C2,C3 creatorNode
    class PA adminNode
```
